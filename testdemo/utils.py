...
def toTensor(img):
    ...
    img = nn.functional.interpolate(img, size=(272, 272), mode='bilinear', align_corners=False)
    return img
...
class EDGLoss(torch.nn.Module):
    def __init__(self):
        super(EDGLoss, self).__init__()
        self.kernel = EDGLoss.get_kernel()
        self.loss = nn.L1Loss(reduction = 'none')
        self.to_range_0_1 = lambda x: (x + 1.) / 2.

    def get_kernel(ksize=3, s=3, self_sub=True):
        c = ksize // 2
        X = torch.arange(0, ksize) - c
        Y = torch.arange(0, ksize) - c
        x, y = torch.meshgrid(X, Y)
        gauss_1 = torch.exp(- (x ** 2 + y ** 2) / (2 * s ** 2))
        gauss_1 = gauss_1 / gauss_1.sum()
        if self_sub:
            gauss_1 = - gauss_1
            gauss_1[c, c] += 1
        return gauss_1.view(1, 1, ksize, ksize)

    def image_enhance(img, kernel):
        p = kernel.shape[-1] // 2
        kernel = kernel.to(device)
        img_edge = F.conv2d(img, kernel)
        img_edge = F.pad(img_edge, (p, p, p, p))
        img_edge = (img_edge - img_edge.min()) / (img_edge.max() - img_edge.min())
        return img_edge

    def forward(self, img0, img1, mask):
        img0 = self.to_range_0_1(img0)
        img1 = self.to_range_0_1(img1)

        img_edge1 =  EDGLoss.image_enhance(img0, self.kernel)
        img_edge2 =  EDGLoss.image_enhance(img1, self.kernel)

        if mask != None:
            mask = 1-mask
            edg_loss = self.loss(img_edge1*mask, img_edge2*mask)
            edg_loss = torch.sum(edg_loss) / torch.count_nonzero(mask)
        else:
            edg_loss = torch.mean(self.loss(img_edge1, img_edge2))
